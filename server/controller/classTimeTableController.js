...
export const getAllSemesters = async (req, res) => {
  try {
    ...
    res.json(sems.map(s => ({
      ...
      Year: s.year || Math.ceil((s.semesterNumber || 1) / 2)
    })));
  } catch (err) { res.status(500).json({ error: err.message }); }
};
...
export const getAllYears = async (req, res) => {
  try {
    const sems = await Semester.find().select('semesterNumber year').sort({ semesterNumber: 1 });
    const years = [...new Set(sems.map(s => s.year || Math.ceil((s.semesterNumber || 1) / 2)))];
    res.json(years.sort((a, b) => a - b).map(y => ({ Year: y })));
  } catch (err) { res.status(500).json({ error: err.message }); }
};
