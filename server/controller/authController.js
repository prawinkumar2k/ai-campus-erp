...
export const login = async (req, res) => {
  try {
    ...
    if (role.toLowerCase() === 'student') {
      ...
      // Support both plain-text and bcrypted passwords for compatibility
      let isPasswordValid = false;
      if (password === dbUser.password) {
        // Plain-text password match (for existing imported data)
        isPasswordValid = true;
      } else {
        // Try bcrypt in case password was hashed
        isPasswordValid = await bcrypt.compare(password, dbUser.password || '').catch(() => false);
      }
