...
const toResponse = (doc) => {
	...
	const result = {
		id: d._id, _id: d._id,
		Staff_Id: d.staffId, Staff_ID: d.staffId, Staff_Name: d.staffName,
		Academic_Year: d.academicYear, Sem_Type: d.semType,
		Course_Name: d.courseName, Dept_Name: d.deptName, Dept_Code: d.deptCode,
		Semester: d.semester, Regulation: d.regulation,
	};
	for (let n = 1; n <= 7; n++) {
		result[`Sub${n}_Name`] = d[`sub${n}Name`];
		result[`Sub${n}_Code`] = d[`sub${n}Code`];
		result[`Sub${n}_Dept_Code`] = d[`sub${n}DeptCode`];
		result[`Sub${n}_Dept_Name`] = d[`sub${n}DeptName`];
		result[`Sub${n}_Semester`] = d[`sub${n}Semester`];
		result[`Sub${n}_Regulation`] = d[`sub${n}Regulation`];
	}
	return result;
};
...
export const getAllocations = async (req, res) => {
	try {
		...
		res.json(allocations.map(toResponse));
