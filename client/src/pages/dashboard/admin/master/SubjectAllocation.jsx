...
const SubjectAllocation = () => {
  ...
  const loadAllocationIntoForm = (rec) => {
    setEditId(rec._id || rec.id || rec.ID || rec.Id || null);
  ...
  useEffect(() => {
    ...
    const fetchMasters = async () => {
      try {
        ...
        if (sRes.ok) setStaffs(toArr(await sRes.json()));
        if (ayRes.ok) setAcademicYears(toArr(await ayRes.json()));
        if (cRes.ok) setCourses(toArr(await cRes.json()));
        if (semRes.ok) setSemesterOptions(toArr(await semRes.json()));
        if (regRes.ok) setRegulationOptions(toArr(await regRes.json()));
      } catch (err) {
